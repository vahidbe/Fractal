 ...
 */
void sbuf_insert(sbuf_t *sp, struct fractal* item)
{
	sem_wait(&(sp->slots));
	sem_wait(&(sp->mutex));
...
{	
	...
	sbuf_init(buff, (numberThreads));
	arguments->buf_arg=buf;
	

	pthread_t prod, cons;	
	pthread_create(&prod, NULL, producer, (void*) arguments);
	pthread_create(&cons, NULL, consumer, (void*) arguments);
